...
public class OAuthRequest {
    ...
    public static <T> RequestModel<T> request(String path, Class<T> model, Object object, HttpMethod method, String... args) {
        switch (method) {
            case GET:
                return new RequestModel<>(path, model, object, args).setOperation((p, m, obj, argz) -> new Request().get(p, m, argz));
            case PUT:
                return new RequestModel<>(path, model, object, args).setOperation((p, m, obj, argz) -> {
                    Request.put(p, obj, argz);
                    return null;
                });
            case POST:
                return new RequestModel<>(path, model, object, args).setOperation((p, m, obj, argz) -> new Request().post(p, m, argz));
            case DELETE:
                return new RequestModel<>(path, model, object, args).setOperation((p, m, obj, argz) -> {
